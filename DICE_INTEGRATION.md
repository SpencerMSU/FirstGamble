...
```json
{
  ...
  "dice_sum": 14,                // Сумма броска (от dice_count до dice_count * 6)
  "dice_count": 3                // Количество кубиков: от 1 до 5 (по умолчанию 1)
}
```
...
3. Проверяется корректность суммы: она должна быть не меньше количества кубиков и не больше `dice_count * 6`.
...
- **400 dice_sum out of range** — сумма не соответствует количеству кубиков (разрешены только 1–5 кубов, сумма не больше `dice_count * 6`).
